...
////////////////////////////////////////////////
Foam::tmp<Foam::fvScalarMatrix> Foam::radiation::radiationModel::ShHat
(
    volScalarField& T,
    const volScalarField& hHat
) const
{
    const volScalarField T3(pow3(T));
    dimensionedScalar As("As",dimEnergy/dimMass/dimTemperature,scalar(1000.));
    return
    (
        Ru()
      - fvm::Sp(4.0*Rp()*T3/As, hHat)
      - Rp()*T3*(T - 4.0*hHat/As)
    );
}
////////////////////////////////////////////////
